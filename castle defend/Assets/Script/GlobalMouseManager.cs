...
public class GlobalMouseManager : MonoBehaviour
{
    ...
    [SerializeField] private float dragSmoothness = 30f; // How smoothly warrior follows mouse (increased for lighter feel)
        ...
        if (hit.collider != null)
        {
            ...
            if (warrior != null && warrior.IsAlive())
            {
                ...
                if (draggedRigidbody != null)
                {
                    ...
                    Debug.Log("Started dragging: " + draggedWarrior.name);
                }
            }
        }
    }
    ...
    void StopDrag()
    {
        if (isDragging && draggedWarrior != null && draggedWarrior.IsAlive())
        {
            // Don't add any throw force - just stop dragging
            draggedWarrior.OnDragStop();
            Debug.Log("Stopped dragging: " + draggedWarrior.name);
        }
        ForceStopDrag();
    }
