...
public class WarriorSpawner : MonoBehaviour
{
    ...
    [SerializeField] private WarriorDataSO warriorDataSO; // Can be assigned in inspector or via SetWarriorDataSO()
    ...
    // Public method to set WarriorDataSO (called by GameManager)
    ...
    private IEnumerator Start()
    {
        ...
        {
            // Assuming your HealthComponent has an OnDeath event
            // You might need to modify this based on your HealthComponent implementation
            castleHealth.OnDeath += OnCastleDestroyed;
        }

        // Don't auto-start level here - let GameManager handle it
        // The GameManager will call SetWarriorDataSO() and then StartLevel()
    }
    ...
    private void RestoreCastleHealth()
    {
        if (castleHealth != null)
        {
            ...
            castleHealth.RestoreToFullHealth(); // You might need to add this method to your HealthComponent
    ...
    private IEnumerator SpawnWarriorsForLevel()
    {
        ...
        // Check if level should end immediately (no active warriors)
    ...
    private void SpawnWarrior(WarriorType type)
    {
        ...
        Debug.Log($"Spawned {type} warrior at position {spawnPosition} ({activeWarriors.Count} active)");
    }
    ...
    private void OnWarriorDestroyed()
    {
        ...
        Debug.Log($"Warrior destroyed. Removed {beforeCount - afterCount} null warriors. Warriors remaining: {activeWarriors.Count}");
        ...
        {
            ForceCleanupWarriors(); // This will automatically run the debug
        }
    ...
    private void CompleteLevel(bool won)
    {
        ...
        if (won)
        {
            ...
            // Don't auto-start next level - let GameManager handle scene transitions
            /*
            ...
            {
                Debug.Log("🎉 ALL LEVELS COMPLETED! GAME WON! 🎉");
    ...
    {
        Debug.Log("=== FORCE CLEANUP STARTED ===");
        Debug.Log($"Active warriors before cleanup: {activeWarriors.Count}");
        ...
        Debug.Log($"Active warriors after cleanup: {activeWarriors.Count}");
        Debug.Log("=== FORCE CLEANUP ENDED ===");
